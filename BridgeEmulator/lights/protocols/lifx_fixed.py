...
try:
    from lifxlan import LifxLAN, Light as LifxLight, Device as LifxDevice, MultiZoneLight, TileChain
...
def _fallback_to_tile_colors(device, matrix_colors, light, zone_colors):
    ...
    try:
        ...
        if tile_width > 8:
            ...
            logging.debug(f"LIFX: Wide tile detected ({tile_width}×{tile_height}), sending {num_chunks} Set64 messages")
            ...
            for chunk_idx in range(num_chunks):
                ...
                logging.debug(f"LIFX: Sent Set64 for x={x_offset}-{x_offset+7}")
